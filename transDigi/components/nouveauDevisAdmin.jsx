...
const NouveauDevisAdmin = () => {
  ...
  useEffect(() => {
    ...
    const baseMax = baseFields.length;
    const optMax = optionalFields.length;
    const percent = Math.round((baseScore / baseMax) * 70 + (optScore / Math.max(optMax,1)) * 30);
    setProgress(Math.max(0, Math.min(100, percent)));
  }, [formData]);

  // Charger la liste des transitaires pour choisir un translataire technique en back
  useEffect(() => {
    (async () => {
      try {
  ...
  const MAX_CLIENT_FILE_SIZE = Number(import.meta.env.VITE_MAX_UPLOAD_FILE_SIZE) || (50 * 1024 * 1024);
      ...
      try {
        ...
        if (!list.length) {
          try {
            ...
            toastError(e?.message || "Impossible de trouver un transitaire technique pour traiter votre demande pour le moment.");
            return;
          }
        }
        const pick = list[0];
        if (pick && pick._id) {
          ...
          toastError('Aucun transitaire technique n\'est disponible pour traiter votre demande pour le moment.');
          return;
        }
        ...
        fd.append('devisOrigin', 'nouveau-devis');
        if (formData.uploadedFiles && formData.uploadedFiles.length) {
        ...
        success('Votre devis a été envoyé à la plateforme (admin) et sera traité avec les transitaires partenaires.');
                  ...
                  <div className="mb-5">
                    ...
                    <div className="border-2 border-dashed rounded-3 p-4 text-center" style={{ borderColor: '#DEE2E6' }}>
                      <input
                        ...
                        multiple
                      />
                      <label htmlFor="fileUploadAdmin" className="cursor-pointer">
                        <div 
                    ...
                    <div className="mb-4">
                      ...
                      <div className="d-flex gap-3 flex-wrap">
                        <div className="form-check">
                          ...
                          />
                          <label className="form-check-label" htmlFor="dangerous-admin">
                        ...
                        <div className="form-check">
                          ...
                          />
                          <label className="form-check-label" htmlFor="fragile-admin">
                            Fragile
                          </label>
                        </div>
                      </div>
...
export default NouveauDevisAdmin;
